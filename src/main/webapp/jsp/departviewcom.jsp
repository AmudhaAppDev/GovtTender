...
<body>
	...
	<table class="table table-xm table-dark">
		...
		<tr class="bg-danger">
			...
			<td><a
				href="action.jsp?cemail=<%=rs.getString(3)%>&amp;&amp;cdate=<%=rs.getString(5)%>&amp;&amp;zone=<%=rs.getString(7)%>&amp;&amp;ctype=<%=rs.getString(9)%>&amp;&amp;address=<%=rs.getString(10)%>"><button
						class="btn btn-primary">Action</button></a></td>
