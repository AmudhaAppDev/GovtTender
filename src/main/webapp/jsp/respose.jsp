	...
	<center>
		<table class="table table-bordered">
			...
			<tr>
				...
				<td><%=s.getString(6)%></td>
				<td><%=s.getString(7)%></td>
				<td><a href="/jsp/qview.jsp?id=<%=s.getString(10)%>"><button
							class="btn btn-primary">View</button></a></td>
				<td><a
					href="appac.jsp?id=<%=s.getString(1)%>&&company=<%=s.getString(8)%>&&request=<%=s.getString(7)%>&&com=<%=s.getString(2)%>"><button
							class="btn btn-primary">APPROVED</button></a></td>
				<td><a
					href="reject.jsp?id=<%=s.getString(1)%>&&company=<%=s.getString(8)%>&&request=<%=s.getString(7)%>&&com=<%=s.getString(2)%>"><button
							class="btn btn-primary">REJECT</button></a></td>
				<%
				}
				%>
